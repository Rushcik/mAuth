...
import dev.rushcik.mauth.command.CommandPermissionState;
import dev.rushcik.mauth.dependencies.DatabaseLibrary;
import dev.rushcik.mauth.migration.MigrationHash;
...
public class Settings extends YamlConfig {
  ...
  public String PREFIX = "mAuth &6>>&f";
  ...
  public static class MAIN {
    ...
    public String TOTP_ISSUER = "mAuth by rushcik";
    ...
    @Comment({
        "Implement the automatic login using the plugin, the mAuth client mod and optionally using a custom launcher",
        "See https://github.com/Elytrium/mAuth-ClientMod"
    })
    ...
    public static class BACKEND_API {
      ...
      @Comment({
          ...
          "Required for PlaceholderAPI expansion to work (https://github.com/UserNugget/mAuth-Expansion)"
      })
  ...
  public static class DATABASE {
    ...
    @Comment({
        ...
        "useSSL=false           - disable SSL (enable on production with a certificate).",
        "characterEncoding=utf8 - use UTF-8 character encoding.",
        "serverTimezone=UTC     - force UTC timezone to avoid date/time issues.",
        "useUnicode=true        - enable Unicode support.",
        "rewriteBatchedStatements=true - batch INSERT/UPDATE for better performance."
    })
    public String CONNECTION_PARAMETERS = "?autoReconnect=true&initialTimeout=1&useSSL=false"
        + "&characterEncoding=utf8&serverTimezone=UTC&useUnicode=true&rewriteBatchedStatements=true";

    @Create
    public DATABASE.MYSQL_POOL MYSQL_POOL;

    @Comment("Connection pool settings (applies to MySQL, MariaDB, PostgreSQL)")
    public static class MYSQL_POOL {

      @Comment("Maximum number of free/idle connections kept in the pool. Increase for high player counts.")
      public int MAX_CONNECTIONS_FREE = 5;

      @Comment("Maximum age of a connection in milliseconds before it is closed and replaced (default: 30 minutes).")
      public long MAX_CONNECTION_AGE_MILLIS = 1800000L;

      @Comment("How often (in ms) the pool checks and cleans up stale connections (default: 60 seconds).")
      public long CHECK_CONNECTIONS_EVERY_MILLIS = 60000L;

      @Comment("If true, the pool will test each connection before returning it to avoid using broken connections.")
      public boolean TEST_BEFORE_GET = true;
    }
  }
